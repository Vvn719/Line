...
import template from '../templates/tpflex.js'
...
export default async (event) => {
  try {
    ...
    $('#site-content article').each(function () {
      ...
      bubble.body.contents[0].text = $(this).find('.entry-header a').text().split('台北市集-').pop().split('-2').shift().trim()
      ...
      bubble.body.contents[2].text = $(this).find('.post-tags').text().replaceAll('\n', '').replaceAll('\t', '').replaceAll('標籤', '').replaceAll('@', '').trim()
